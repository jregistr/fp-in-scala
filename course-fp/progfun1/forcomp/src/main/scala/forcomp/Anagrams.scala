...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    ...
    val grouped: Map[Int, List[Char]] = lower.toList.groupBy(char => lower.count(_ == char))
    (for {
      (count: Int, characters: List[Char]) <- grouped
      character: Char <- characters
    } yield character -> count).toList.sortBy(_._1)
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    case Nil => List(Nil)
    case h :: t => h match {
      case (char: Char, count: Int) =>
        val tc: List[Occurrences] = combinations(t)
        (for {
          combination: Occurrences <- tc
          n <- 1 to count
        } yield (char -> n) :: combination) ::: tc
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = y.foldLeft(x.toMap) {
    case (acc: Map[Char, Int], (charY, occInY)) =>
      acc.get(charY) match {
        case None => acc
        case Some(occInAcc) =>
          val c = occInAcc - occInY
          if (c > 0) {
            acc.updated(charY, c)
          } else {
            acc - charY
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def loop(occurrences: Occurrences): List[Sentence] = occurrences match {
      ...
      case _ :: _ =>
        for {
          combinations: Occurrences <- combinations(occurrences)
          word: Word <- dictionaryByOccurrences.getOrElse(combinations, Nil)
